...
void psh::GroupCommon::OnUpdate(float milli)
{
    UpdateContent(milli);
    _fps++;
...
bool psh::GroupCommon::GetClosestTarget(FVector location, weak_ptr<psh::ChatCharacter>& target)
{
    bool find = false;
    float closest = 99999;
...
void psh::GroupCommon::SendCreateAndGetInfo(const shared_ptr<psh::GameObject>& object, FVector location, const std::span<const psh::Sector> offsets, bool isSpawn)
{
    ...
    //실행시킨다. 
    ranges::for_each(addSectors,[this,&createThis,&toCreate,&object](flat_unordered_set<shared_ptr<ChatCharacter>> sector){
         for(auto& player : sector)
         {
              SendPacket(static_pointer_cast<Player>(player)->SessionId(),createThis);
              ...
              if(object->ObjectGroup() == eCharacterGroup::Player)
                                            ...
                                            const std::span<const psh::Sector> offsets, bool isDead)
{
    ...
    //실행시킨다. 
    ranges::for_each(delSectors,[this,&deleteThis,&toDelete,isDead,&object](flat_unordered_set<shared_ptr<ChatCharacter>> sector){
         for(auto& player : sector)
         {
              SendPacket(static_pointer_cast<Player>(player)->SessionId(),deleteThis);
